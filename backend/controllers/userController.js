...
exports.resetPassword = catchAsyncError(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new ErrorHandler("Reset token is invalid or have been expired", 400)
    );
  }

  if (req.body.password !== req.body.confirmPassword) {
    return next(new ErrorHandler("Not match password. Pls check again", 400));
  }
...
exports.deleteUser = catchAsyncError(async (req, res, next) => {
  ...
  res.status(200).json({
    success: true,
    message: "This user has been deleted away from system ",
  });
});
